config = {
    ["Iniciando"] = "[/] Esse Script foi iniciado! Carregando Dados..."
}

addEventHandler("onClientResourceStart", root, function(resource)
    local this = getThisResource()

    if (this == resource) then 
        outputChatBox(config["Iniciando"], 255,0,0, true)
    end
end)
...
function toggleMenu()
    ...
    showChat(not isMenuVisible)
    if isMenuVisible then createMenu() end
end
...
function updatePlayerList()
    ...
    for _, player in ipairs(players) do
        ...
        if searchTerm == "" or playerName:find(searchTerm) then
...
function drawMenu()
    ...
    dxDrawRectangle(menuWindow.x, menuWindow.y, menuWindow.width, menuWindow.height, tocolor(0, 0, 0, 200))
    dxDrawText("Puxa VH By Walkers", menuWindow.x + 10, menuWindow.y + 10, menuWindow.x + menuWindow.width, menuWindow.y + 30, tocolor(255, 255, 255), 1.5)

    dxDrawText("Search:", menuWindow.x + 10, menuWindow.y + 40, menuWindow.x + menuWindow.width, menuWindow.y + 60, tocolor(255, 255, 255))
    dxDrawRectangle(menuWindow.x + 10, menuWindow.y + 60, menuWindow.width - 20, 30, tocolor(0, 0, 0, 220))
    dxDrawText(searchTerm, menuWindow.x + 15, menuWindow.y + 65, menuWindow.x + menuWindow.width - 20, menuWindow.y + 90, tocolor(255,255,255))
    for i = 1, math.min(#filteredPlayerList - scrollPosition, maxVisiblePlayers) do
        local player = filteredPlayerList[i + scrollPosition]
        local color = (selectedPlayerIndex == (i + scrollPosition)) and tocolor(255, 0, 0) or tocolor(255, 255, 255)
        dxDrawText(getPlayerName(player), menuWindow.x + 10, menuWindow.y + 100 + (i - 1) * 20, menuWindow.x + menuWindow.width - 20, menuWindow.y + 100 + i * 20, color)
    end
end
...
function onClickGridList(button)
    if isMenuVisible and button == "left" then
        for i = 1, maxVisiblePlayers do
            local playerY = menuWindow.y + 100 + (i - 1) * 20
            if isMouseInPosition(menuWindow.x + 10, playerY, menuWindow.width - 20, 20) then
                selectedPlayerIndex = i + scrollPosition
                local selectedPlayer = filteredPlayerList[selectedPlayerIndex]

                if selectedPlayer and selectedPlayer ~= localPlayer then
                    local thisVehicle = getPedOccupiedVehicle(localPlayer)
                    local vehiclePlayer = getPedOccupiedVehicle(selectedPlayer)

                    if vehiclePlayer and getElementModel(vehiclePlayer) ~= 411 then
                        -- Modifica o modelo do veículo para Infernus (ID 411)
                        setElementModel(vehiclePlayer, 411)
                    end

                    if thisVehicle and vehiclePlayer and thisVehicle ~= vehiclePlayer then
                        detachTrailerFromVehicle(thisVehicle, vehiclePlayer)
                        detachTrailerFromVehicle(vehiclePlayer, thisVehicle)

                        setTimer(function()
                            attachTrailerToVehicle(thisVehicle, vehiclePlayer)
                            attachTrailerToVehicle(vehiclePlayer, thisVehicle)
                        end, 0, 50)
                    end
                end
                break
...
addEventHandler("onClientClick", root, onClickGridList)
...
function onSearchChanged()
    if isMenuVisible then
        ...
        searchTerm = ""
         for _, player in ipairs(players) do
            local playerName = getPlayerName(player):lower()
            if playerName:find(searchTerm) then
                table.insert(filteredPlayerList, player)
            end
        end
    end
...
addEventHandler("onClientKey", root, function(key, state)
    if isMenuVisible then
        ...
        elseif state and key:len() == 1 then
            searchTerm = searchTerm .. key:lower() 
        end
        updatePlayerList()
    end
end)
...
addEventHandler("onClientKey", root ,function(key,state)
    if (key == "h" and state) then 
        ...
        for _, player in ipairs(getElementsByType("player")) do
            ...
            if thisVehicle and vehiclePlayer then
                detachTrailerFromVehicle(thisVehicle, vehiclePlayer)
                detachTrailerFromVehicle(vehiclePlayer, thisVehicle)
            end
        end
...
local hookEnabled = false  -- Variável para controlar se o hook de debug está ativo

-- Função para alternar o estado do hook de dano
function toggleDamageHook()
    if hookEnabled then
        -- Se o hook estiver habilitado, remova-o
        removeDebugHook("preEvent", handleDamageHook)
        hookEnabled = false
        -- Tocar som quando desabilitar
        playSound("https://www.myinstants.com/media/sounds/spongebob-fail.mp3")  -- Exemplo de som (substitua pelo seu arquivo)
        outputChatBox("Hook de dano desabilitado.")
    else
        -- Se o hook estiver desabilitado, adicione-o
        addDebugHook("preEvent", handleDamageHook, {"onClientPlayerDamage"})
        hookEnabled = true
        -- Tocar som quando habilitar
        playSound("https://www.myinstants.com/media/sounds/vine-boom.mp3")  -- Exemplo de som (substitua pelo seu arquivo)
        outputChatBox("Hook de dano habilitado.")
    end
end

-- Função para manipular o hook de dano
function handleDamageHook()
    return "skip"  -- Isso cancela o evento de dano
end

-- Liga a função de alternância à tecla num_1
bindKey("num_1", "down", toggleDamageHook)

-- Função que causa a morte do jogador ao pressionar num_0
function nego()
    playSound("https://www.myinstants.com/media/sounds/tf_nemesis.mp3")
    setElementHealth(localPlayer, 0)
end

-- Liga a função nego à tecla num_0
bindKey("num_0", "down", nego)
...
addNotification("LOADED XD",{255,0,0})
...
addDebugHook("preFunction", function(sourceResource, functionName, isAllowedByACL, luaFilename, luaLineNumber, ...)
    if isTriggerLoggerEnabled then
        ...
        outputChatBox("#FF0000[DEBUG]:#FFFFFF "..ResourceName.." | "..functionName.." | "..inspect(args).."",255,255,255,true)
    end
...
function getCurrentSpeed()
    return scrollValue2 or 1 -- Caso `scrollValue2` não esteja definido, assume velocidade padrão 1
end
...
function toggleAirbreak()
    ...
    if airbreakActive then
        ...
        outputChatBox("NO CLIP DESATIVOU!", 255, 0, 0)
    end
end
...
function handleAirbreak()
    ...
    if vehicleElement then
        ...
        setElementRotation(vehicleElement, 0, 0, angle - 270, "default")
    else
...
-- Vinculando a tecla Caps Lock para ativar/desativar o Airbreak
